...
public class CatalogController {
  ...
  /**
   * Public navbar search ("Поиск планов…"). Case-insensitive substring match on service name,
   * scoped to active services. Short queries return an empty list silently — see {@link
   * CatalogService#searchServices(String, int)}.
   */
  @GetMapping("/search")
  ...
  /**
   * FIFO match for the subscription flow: tells the FE whether to push the user into an existing
   * room (oldest OPEN with a free seat the user doesn't own) or to start a new one. Auth required
   * so we can exclude the caller's own rooms from the match — anonymous "should I join?" doesn't
   * make sense.
   */
  ...
  /**
   * Public service-logo proxy. Mirrors the avatar / news-image streamers: the storage service
   * validates the filename so it can only resolve to a key under the {@code service-logos/} prefix.
   */
