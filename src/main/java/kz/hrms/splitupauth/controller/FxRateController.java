...
public class FxRateController {
    ...
    /**
     * Public live-conversion source for the SPA. Cache-Control lets the
     * browser / CDN reuse the snapshot for {@code app.fx.response-cache-seconds}
     * — the upstream refresh cadence (default 6h) leaves plenty of room for
     * intermediate caching without serving stale data.
     */
    @GetMapping("/rates")
    public ResponseEntity<FxRatesResponse> rates() {
        FxRatesResponse body = FxRatesResponse.builder()
                .base("KZT")
