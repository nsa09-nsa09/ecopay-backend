...
/**
 * Editorial news item surfaced on the landing page. Tri-lingual copy (kk/ru/en)
 * mirrors the V30 site_content shape — every field is optional per language so
 * the admin can publish a partially translated draft.
 *
 * <p>{@code imageKey} is the S3 object key written by {@code NewsImageStorageService};
 * never an absolute URL. The bytes are streamed back through the backend just
 * like avatars (see {@code PublicAvatarController}).</p>
 */
@Entity
