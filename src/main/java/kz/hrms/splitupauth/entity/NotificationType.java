 ...
 *
 * <ul>
 *   <li>{@link #category} — the grouping shown in the preferences UI, so users
 *       toggle a small set of buckets rather than ~20 individual types.</li>
 *   <li>{@link #emailEligible} — whether this type may ever be sent over email.
 *       Low-signal events (e.g. a new applicant) are in-app only regardless of
 *       the user's email preference; high-importance ones (payment failed,
 *       dispute resolved, payout, ban) can additionally email.</li>
 * </ul>
 *
 * The actual decision to deliver on a channel is preference ∧ eligibility — see
