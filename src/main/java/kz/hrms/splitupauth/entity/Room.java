...
@Table(
    ...
    indexes = {
      ...
      @Index(
          name = "idx_rooms_service_status_start_date",
          columnList = "service_id,status,start_date")
    })
@Data
@Builder
@NoArgsConstructor
...
public class Room {
  ...
  /**
   * Lazy-minted on the first GET /rooms/{id}/invite-link by the owner. Stays null until then so
   * legacy rooms don't need a backfill — the partial unique index (V36) ignores null entries.
   */
