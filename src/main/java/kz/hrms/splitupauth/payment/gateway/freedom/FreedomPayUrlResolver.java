...
/**
 * Resolves the absolute URLs that are handed to Freedom Pay (webhook result URLs and
 * browser success/failure redirects).
 *
 * <p>The configured values (see {@code application.properties}) default to {@code localhost}
 * so local dev works out of the box, but those defaults must never leak into a deployed
 * environment. This resolver derives the real host from the current HTTP request — the same
 * approach {@code AvatarStorageService} uses for avatar links — and only falls back to the
 * configured value when it is an explicit, non-loopback URL (i.e. an operator deliberately
 * set {@code FREEDOMPAY_*_URL}). The reverse proxy's {@code X-Forwarded-*} headers are honoured
 * via {@code server.forward-headers-strategy=framework}, so behind nginx this yields the public
 * https host automatically.</p>
 *
 * <p>Resolution priority:
 * <ol>
 *   <li>an explicit, non-loopback configured/override URL (deliberate ops setting);</li>
 *   <li>otherwise, a URL built from the current request host;</li>
 *   <li>as a last resort (no request in scope), the configured value as-is.</li>
 * </ol></p>
 */
@Component
...
public class FreedomPayUrlResolver {
    ...
    private String resolveBackend(String configured, String path) {
        ...
        log.warn("No request in scope and no explicit Freedom Pay URL configured; "
                + "falling back to '{}' (set FREEDOMPAY_*_URL in deployed environments)", configured);
    ...
    /**
     * Origin of the SPA that initiated the payment. Prefers the {@code Origin} header, then
     * {@code Referer}; both reflect where the user's browser actually is, so the redirect lands
     * back on the same frontend (localhost in dev, the public domain in prod).
     */
