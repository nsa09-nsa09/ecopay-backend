...
/**
 * Publishes account-lifecycle events (ban / unban / forced logout) to the affected user's personal
 * STOMP topic. The interceptor in {@link WebSocketAuthChannelInterceptor#validateAccountTopic}
 * guarantees only the owner can subscribe.
 */
@Service
...
public class AccountRealtimeService {
  ...
  public void publishUnbanned(Long userId) {
    AccountEventDto event =
        AccountEventDto.builder().type("UNBANNED").occurredAt(LocalDateTime.now()).build();
    messagingTemplate.convertAndSend(topicFor(userId), event);
  }
}
