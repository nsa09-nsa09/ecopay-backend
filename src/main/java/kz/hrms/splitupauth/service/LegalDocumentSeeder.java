 ...
 *
 * <p>For each {@link LegalDocument.DocType}, if the row does not already exist,
 * it is created with {@code version=1} and the body loaded from the
 * classpath-bundled {@code legal/*.md} resource files (one per language). If
 * the row already exists it is left untouched — this way an admin's edits
 * survive redeploys.
 */
@Component
...
public class LegalDocumentSeeder {
    ...
    private void seedIfMissing(LegalDocument.DocType docType,
                               String titleKz,
                               String titleRu,
                               String titleEn,
