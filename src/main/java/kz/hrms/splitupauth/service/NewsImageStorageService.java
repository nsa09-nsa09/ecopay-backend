...
/**
 * Handles the news-image upload pipeline end-to-end (mirrors
 * {@link AvatarStorageService}): input validation (size, extension, MIME magic
 * bytes), decoding, downscaling to a target width, re-encoding as JPEG (which
 * strips any embedded payload), and persistence to S3-compatible object
 * storage (Cloudflare R2 in this deployment).
 *
 * <p>Only the <b>object key</b> (e.g. {@code news/ab12...jpg}) is returned and
 * persisted on {@code News.imageKey} — never an absolute URL. Images are
 * served <b>through the backend host</b> ({@link #publicUrl(String)} +
 * {@link #loadImageBytes(String)}) so R2 credentials and the bucket endpoint
 * never leave the backend, exactly like avatars.</p>
 */
@Service
...
public class NewsImageStorageService {
    ...
    /**
     * Validates the upload, normalises it to a width-bounded JPEG and stores
     * it in the bucket. Returns the object key persisted on {@code News.imageKey}.
     */
    public String store(MultipartFile file) {
        ...
        try {
            ...
            log.error("Failed to upload news image to bucket {} key {}",
     ...
     * ({@code {host}/api/v1/news/images/<file>.jpg}). Non-managed values
    ...
    /**
     * Streams a stored image's bytes from the bucket for the public serving
     * endpoint. The filename is validated so it can only resolve to a key
     * under our news prefix. Throws {@link ResourceNotFoundException} when missing.
     */
    public byte[] loadImageBytes(String filename) {
        ...
        try {
            ...
            log.warn("Failed to read news image {} from bucket {}: {}",
    ...
    /**
     * Deletes a previously stored news image from the bucket. Non-managed
     * values (null, or leftover legacy strings) are ignored.
     */
    public void deleteIfManaged(String storedValue) {
        ...
        try {
            ...
            log.warn("Failed to delete news image {} from bucket {}: {}",
    ...
    private boolean isManaged(String storedValue) {
        return storedValue != null
                && !storedValue.isBlank()
                && storedValue.startsWith(NEWS_PREFIX);
    }
    ...
    /**
     * Width-bounded downscale: keeps the aspect ratio, shrinks to {@code targetWidth}
     * on the long edge when necessary, leaves anything smaller untouched.
     */
