 ...
 *
 * <ul>
 *   <li>Public feed — only PUBLISHED, sorted by sort_order then published_at desc.
 *   <li>Admin list — every status, newest-first, optional status filter.
 * </ul>
 *
 * Every admin write logs to {@code admin_action_log} via {@link
 * AdminActionType#NEWS_CREATED}/{@code NEWS_UPDATED}/{@code NEWS_DELETED}. Image upload/replace and
 * delete-time cleanup are delegated to {@link NewsImageStorageService}; this service never talks to
 * S3 directly.
 */
@Service
...
public class NewsService {
  ...
  public PagedResponse<NewsDto> publicList(int page, int size) {
    ...
    Page<News> result =
        newsRepository.findByStatusOrderBySortOrderAscPublishedAtDesc(
  ...
  public NewsDto create(User admin, CreateNewsRequest req, HttpServletRequest http) {
    ...
    auditWriter.writeOrSwallow(
        admin, AdminActionType.NEWS_CREATED, news.getId(), null, newState, http);
  ...
  /**
   * Hard delete + remove the attached S3 object if any. We pick hard delete over a soft "ARCHIVED"
   * status because archive already exists as an editorial state — making delete also soft would
   * leave admins with no way to actually remove an item. The audit row keeps the trail.
   */
  @Transactional
  ...
  private void requireAnyTitle(News news) {
    boolean hasTitle =
        isNonBlank(news.getTitleKz())
            || isNonBlank(news.getTitleRu())
            || isNonBlank(news.getTitleEn());
    if (!hasTitle) {
      throw new InvalidRequestException("Хотя бы один заголовок (kz/ru/en) должен быть заполнен");
    }
  }
