 ...
 * auto-refund it. The resulting token is saved and registered as the owner's payout method —
 * the owner never sees or types a token. This flow is deliberately isolated from the
 * room-payment {@code PaymentIntent} path (which is bound to a room member).</p>
 */
@Service
...
public class PayoutCardBindingService {
    ...
    /**
     * Starts a binding: creates a verification charge on the hosted page and returns the URL
     * the owner must visit to enter their card. {@code returnUrl} is the frontend page the
     * provider should redirect back to; the binding id and status are appended to it.
     */
    @Transactional
    public PayoutCardBindingResponse initBinding(User user, String returnUrl) {
        ...
        PayoutCardBinding binding = bindingRepository.save(PayoutCardBinding.builder()
                .user(user)
        ...
        try {
            resp = gateway.initCharge(GatewayChargeRequest.builder()
            ...
            return PayoutCardBindingResponse.builder()
                    .bindingId(binding.getId()).status("FAILED")
                    .failureMessage(binding.getFailureMessage()).build();
        }
        ...
        if (!resp.isSuccess()) {
            ...
            return PayoutCardBindingResponse.builder()
                    .bindingId(binding.getId()).status("FAILED")
                    .failureMessage(resp.getFailureMessage()).build();
        }
    ...
    /**
     * Confirms a binding after the owner returns from the hosted page: queries the provider for
     * the card token, saves it, registers the payout method, and refunds the verification charge.
    ...
    public PayoutCardBindingConfirmResponse confirmBinding(User user, Long bindingId) {
        PayoutCardBinding binding = bindingRepository.findByIdAndUser(bindingId, user)
        ...
        if ("SUCCESS".equals(binding.getStatus())) {
            return PayoutCardBindingConfirmResponse.builder()
                    .status("SUCCESS")
                    .method(binding.getPayoutMethod() == null ? null : PayoutMethodDto.from(binding.getPayoutMethod()))
                    .build();
        }
        if ("FAILED".equals(binding.getStatus())) {
            return PayoutCardBindingConfirmResponse.builder()
                    .status("FAILED").message(binding.getFailureMessage()).build();
        }
        ...
        if ("SUCCESS".equals(status.getStatus())) {
            ...
            return PayoutCardBindingConfirmResponse.builder()
                    .status("SUCCESS").method(PayoutMethodDto.from(method)).build();
        }
        ...
        if ("FAILED".equals(status.getStatus())) {
            ...
            bindingRepository.save(binding);
            return PayoutCardBindingConfirmResponse.builder()
                    .status("FAILED").message(status.getFailureMessage()).build();
        }
    ...
    private PayoutMethod completeBinding(PayoutCardBinding binding, User user, String token, String panMask) {
        ...
        try {
            gatewayRegistry.defaultGateway().refund(GatewayRefundRequest.builder()
            ...
            log.warn("Card binding {} verification refund failed (manual reconcile needed): {}",
