...
public class AdminDashboardService {
    ...
    public AdminDashboardKpisDto getKpis() {
        ...
        long newUsersLast30Days = singleLong(
                "SELECT COUNT(u) FROM User u WHERE u.createdAt >= ?1", thirtyDaysAgo);
        ...
        long uniqueVisitorsToday = jdbcCountLong(
                "SELECT COUNT(*) FROM site_visit WHERE visit_date = ?", today);
        long uniqueVisitors30d = jdbcCountLong(
                "SELECT COUNT(DISTINCT visitor_id) FROM site_visit WHERE visit_date >= ?", from30);
        long totalPageViews30d = jdbcCountLong(
                "SELECT COALESCE(SUM(page_count), 0) FROM site_visit WHERE visit_date >= ?", from30);

        // ----- Member rollups -----
        long totalActiveMembers = singleLong(
                "SELECT COUNT(m) FROM RoomMember m WHERE m.deletedAt IS NULL AND m.status = ?1",
                MemberStatus.ACTIVE);
        double avgMembersPerRoom = totalRooms == 0
                ? 0.0
        ...
        BigDecimal totalActiveSubscriptionsValueKzt = jdbcBigDecimal(
        ...
        long newRoomsLast30Days = singleLong(
                "SELECT COUNT(r) FROM Room r WHERE r.deletedAt IS NULL AND r.createdAt >= ?1",
                thirtyDaysAgo);

        double conversionVisitorToUser30d = uniqueVisitors30d == 0
                ? 0.0
                : Math.round(((double) newUsersLast30Days / uniqueVisitors30d) * 10000.0) / 100.0;
        double refundRatePercent = totalRevenue == null || totalRevenue.signum() == 0
                ? 0.0
                : totalRefunds.divide(totalRevenue, 4, RoundingMode.HALF_UP)
                        .multiply(BigDecimal.valueOf(100))
                        .setScale(2, RoundingMode.HALF_UP)
                        .doubleValue();

        long openTickets = singleLong(
        ...
        double avgRoomFillRate = avgFill == null ? 0.0
                : Math.round(avgFill * 10000.0) / 10000.0;
    ...
    public DashboardMetricsDto getMetrics(String granularity, LocalDate from, LocalDate to) {
        ...
        jdbc.query(visitSql, rs -> {
            ...
            cur[3] += visitors;
            cur[4] += views;
        }, unit, sqlLabelPattern(unit), java.sql.Date.valueOf(resolvedFrom.toLocalDate()),
                java.sql.Date.valueOf(resolvedTo.toLocalDate()));

        String roomSql = "SELECT to_char(date_trunc(?, created_at), ?) AS bucket, COUNT(*) "
                + "FROM rooms WHERE deleted_at IS NULL AND created_at >= ? AND created_at < ? "
                + "GROUP BY bucket ORDER BY bucket";
        jdbc.query(roomSql, rs -> {
            String period = rs.getString(1);
            long count = rs.getLong(2);
            long[] cur = buckets.computeIfAbsent(period, k -> new long[6]);
            cur[5] += count;
        ...
        String revenueSql = "SELECT to_char(date_trunc(?, t.created_at), ?) AS bucket, "
                ...
                + "GROUP BY bucket ORDER BY bucket";
        jdbc.query(revenueSql, rs -> {
            String period = rs.getString(1);
            BigDecimal revenue = rs.getBigDecimal(2);
            revenueBuckets.merge(period, revenue == null ? BigDecimal.ZERO : revenue, BigDecimal::add);
        }, unit, sqlLabelPattern(unit), fromTs, toTs);
