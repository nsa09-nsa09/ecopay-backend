 ...
 *
 * <p>Lookups for KZT always return 1. For the rest we fetch USD-base rates once a day from a free
 * public source (open.er-api.com) and derive the KZT-base figures by dividing each X/USD by
 * KZT/USD. If the upstream is unreachable, we fall back to a hand-curated table so the product
 * stays usable and tests never depend on a network round-trip.
 */
@Slf4j
@Service
...
public class ExchangeRateService {
  ...
  private static final Map<String, BigDecimal> FALLBACK_RATES_TO_KZT =
      Map.of(
          "KZT", BigDecimal.ONE,
  ...
  /**
   * Snapshot of rates as "1 currency unit = X KZT". The map always contains every {@link Currency}
   * (fallback used for anything the upstream omits).
   */
  ...
  /**
   * Returns the snapshot rate currently in effect for {@code currency}; falls back to 1 for
   * unknowns.
   */
  ...
  /**
   * Refresh from upstream. Safe to call repeatedly; failures leave the previous snapshot in place.
   */
  public void refresh() {
    try {
      HttpClient client =
      ...
      if (response.statusCode() < 200 || response.statusCode() >= 300) {
        log.warn(
            "FX refresh: upstream returned status {}; keeping previous snapshot",
            response.statusCode());
        return;
      }
  ...
  /**
   * Parses an USD-base response (rates[USD]=1, rates[KZT]=455, ...) into our KZT-base form by
   * dividing each currency's USD rate by the KZT rate. Visible for tests so they can exercise the
   * conversion without a network.
   */
      ...
      for (Currency c : Currency.values()) {
        ...
        if (node != null && !node.isNull()) {
          ...
          if (perUsd.signum() > 0) {
            ...
            BigDecimal perX =
                kztPerUsd.divide(perUsd, MathContext.DECIMAL64).setScale(6, RoundingMode.HALF_UP);
            next.put(code, perX);
            continue;
          }
        }
