 ...
 *
 * <p>Storage is sparse: a row exists only once a user diverges from the all-on
 * default, so {@link #channelsFor} treats a missing row as "both channels on".
 * The {@code email} flag is always gated by {@link NotificationType#isEmailEligible()}
 * — a user can't opt into email for an in-app-only type.
 */
@Service
...
public class NotificationPreferenceService {
    ...
    @Transactional
    public List<NotificationPreferenceDto> update(User user, UpdateNotificationPreferenceRequest request) {
        for (UpdateNotificationPreferenceRequest.Item item : request.getPreferences()) {
            NotificationType type = item.getType();
            NotificationPreference pref = preferenceRepository.findByUserAndType(user, type)
