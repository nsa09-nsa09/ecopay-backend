...
/**
 * Handles the avatar upload pipeline end-to-end: input validation (size, extension, MIME, magic
 * bytes), decoding, downscaling, re-encoding (which strips any embedded payload), and persistence
 * to S3-compatible object storage (Cloudflare R2 in this deployment).
 *
 * <p>Only the <b>object key</b> (e.g. {@code avatars/ab12...jpg}) is returned and persisted on
 * {@code User.avatar} — never an absolute URL. Images are served <b>through the backend host</b>:
 * {@link #publicUrl(String)} builds a {@code {host}/api/v1/users/avatars/<file>} link, and the
 * public controller streams the bytes from R2 via {@link #loadAvatarBytes(String)}. R2 credentials
 * and the bucket endpoint never leave the backend. The host is taken from the current request (so
 * links auto-match localhost or the public domain with no config — honouring {@code X-Forwarded-*}
 * behind the reverse proxy), falling back to {@code app.base-url} only when there is no request in
 * scope.
 *
 * <p>All uploaded avatars are stored as JPEG with a UUID-based key; the original filename and
 * Content-Type from the client are never trusted.
 */
@Service
...
public class AvatarStorageService {
  ...
  /**
   * Object-key "folder" for avatars within the bucket. This is an application-structure constant,
   * not an env knob — new media types (tickets/, services/, ...) get their own prefix in their own
   * service.
   */
  ...
  /**
   * Validates the upload, normalises it to a 512×512 JPEG and stores it in the bucket. Returns the
   * object key persisted on {@code User.avatar} — never the bucket URL.
   */
   ...
   * {host}/api/v1/users/avatars/<file>.jpg}). Non-managed values (null/blank/legacy) yield {@code
   * null}.
   */
  ...
  /**
   * Host the avatar link is built against. Prefers the current request's scheme/host so the link
   * auto-matches localhost or the public domain (the reverse proxy's {@code X-Forwarded-*} headers
   * are honoured via {@code server.forward-headers-strategy}). Falls back to {@code app.base-url}
  ...
  /**
   * Streams a stored avatar's bytes from the bucket for the public serving endpoint. The filename
   * is validated so it can only resolve to a key under our avatar prefix. Throws {@link
  ...
  public byte[] loadAvatarBytes(String filename) {
    ...
    try {
      ...
      log.warn(
          "Failed to read avatar {} from bucket {}: {}",
          key,
  ...
  /**
   * Deletes a previously stored avatar from the bucket. Non-managed values (null, or leftover
  ...
  public void deleteIfManaged(String storedValue) {
    ...
    try {
      ...
      log.warn(
          "Failed to delete avatar {} from bucket {}: {}",
          storedValue,
          s3Properties.getBucket(),
          ex.getMessage());
    }
  ...
  private boolean isManaged(String storedValue) {
    return storedValue != null && !storedValue.isBlank() && storedValue.startsWith(AVATAR_PREFIX);
  }
