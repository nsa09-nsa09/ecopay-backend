...
public class SavedCardService {
  ...
  @Transactional
  public SavedCard upsertSavedCard(
      User user, String providerName, String providerToken, String panMask) {
    SavedCard existing =
        repository
            .findByUserAndProviderTokenAndProviderName(user, providerToken, providerName)
            .orElse(null);
    if (existing != null) {
    ...
    boolean firstCard =
        repository.findByUserAndIsDefaultTrueAndStatus(user, SavedCardStatus.ACTIVE).isEmpty();
    SavedCard card =
        SavedCard.builder()
            .user(user)
  ...
  public void setDefault(User user, Long cardId) {
    ...
    repository
        .findByUserAndIsDefaultTrueAndStatus(user, SavedCardStatus.ACTIVE)
        .ifPresent(
            prev -> {
