...
/**
 * Global admin "spotlight" search across the three most-trafficked admin sections (rooms, users,
 * feedback). One round-trip, three small grouped lists — used by the admin shell's top-bar
 * quick-jump.
 *
 * <p>Each group is independently capped by {@code limit}; one underperforming group never starves
 * the others. All queries are ILIKE with a {@code %q%} pattern so they pick up substrings anywhere
 * in the field. Inputs are validated and length-bounded so an empty / oversized query
 * short-circuits back to empty groups rather than scanning the world.
 */
@Slf4j
@Service
...
public class AdminSearchService {
  ...
  public AdminSearchResultDto search(String q, Integer limit) {
    AdminSearchResultDto empty =
        AdminSearchResultDto.builder()
            .rooms(List.of())
            .users(List.of())
            .feedback(List.of())
            .build();
  ...
  /**
   * PG's LIKE treats {@code %} {@code _} {@code \\} as metacharacters. The SQL below uses {@code
   * ESCAPE '\'} so we can safely match literal underscores in identifiers like "user_id_1".
   */
  ...
  private List<AdminSearchResultDto.Item> searchRooms(String pattern, int limit) {
    ...
    try {
      return jdbc.query(
          sql,
          (rs, n) ->
              AdminSearchResultDto.Item.builder()
                  .id(rs.getLong("id"))
                  .type("room")
  ...
  private List<AdminSearchResultDto.Item> searchUsers(String pattern, int limit) {
    ...
    String sql =
        "SELECT id, email, display_name, public_id, phone, status FROM users "
            + "WHERE deleted_at IS NULL "
            + "  AND ( "
            + "        email         ILIKE ? ESCAPE '\\' "
    ...
    try {
      return jdbc.query(
          ...
          (rs, n) -> {
            ...
            String label = (name != null && !name.isBlank()) ? name + " <" + email + ">" : email;
  ...
  private List<AdminSearchResultDto.Item> searchFeedback(String pattern, int limit) {
    String sql =
        "SELECT id, subject, message, status FROM feedback "
    ...
    try {
      return jdbc.query(
          ...
          (rs, n) -> {
            ...
            String status = rs.getString("status");
            String label =
                (subject != null && !subject.isBlank()) ? subject : truncate(message, 80);
            String sub = status;
            return AdminSearchResultDto.Item.builder()
                .id(rs.getLong("id"))
                .type("feedback")
