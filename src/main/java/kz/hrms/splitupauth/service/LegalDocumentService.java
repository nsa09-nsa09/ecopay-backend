...
public class LegalDocumentService {
  ...
  public Integer currentVersion(LegalDocument.DocType docType) {
    return repository.findByDocType(docType).map(LegalDocument::getVersion).orElse(null);
  }

  @Transactional
  public LegalDocumentDto adminUpdate(
      User admin,
  ...
  /**
   * Parse a path-segment value ("terms" | "privacy", any case) into the enum. Rejects unknown
   * values with a 400 instead of 500-ing on IllegalArg.
   */
