...
/**
 * Wires the Spring Boot 4 auto-configured Jackson 3.x JsonMapper (used by HTTP
 * message converters) so every LocalDateTime is serialized as an ISO offset
 * string in Asia/Almaty. The +05:00 suffix is what removes timezone ambiguity
 * for the frontend.
 *
 * <p>The legacy 2.x mapper in {@link JacksonLegacyConfig} is configured the
 * same way so internal code paths that use {@code ObjectMapper objectMapper}
 * stay consistent with the HTTP layer.
 */
@Configuration
