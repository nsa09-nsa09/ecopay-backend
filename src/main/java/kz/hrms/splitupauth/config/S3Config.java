...
/**
 * Wires the S3 client from {@link S3Properties}. Works against real AWS S3 and S3-compatible stores
 * (Cloudflare R2, MinIO, ...). Avatars are streamed through the backend rather than served via
 * pre-signed URLs, so only the sync client is needed. Building the bean performs no network I/O, so
 * the app starts even before the bucket exists or credentials are filled in — failures surface only
