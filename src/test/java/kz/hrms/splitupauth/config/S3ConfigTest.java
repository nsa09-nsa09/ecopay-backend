...
class S3ConfigTest {
  ...
  /**
   * Regression guard: without S3 credentials (CI, local dev without R2) the client bean must still
   * build. {@code AwsBasicCredentials} rejects blank keys, so blank values are swapped for
   * placeholders in {@link S3Config} — otherwise bean creation would fail and bring down the whole
   * context.
   */
  @Test
