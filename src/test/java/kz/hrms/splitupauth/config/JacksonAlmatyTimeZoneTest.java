...
/**
 * Verifies that every LocalDateTime that flies out of the backend in JSON ends
 * with the Almaty offset (+05:00). Without that suffix the frontend has no way
 * to disambiguate the wall clock and historically rendered timestamps five
 * hours behind reality.
 *
 * <p>Runs as a plain unit test (no Spring context) by exercising the legacy
 * Jackson 2.x mapper directly — the same module configuration is mirrored in
 * the Jackson 3.x customizer used by HTTP responses.
 */
class JacksonAlmatyTimeZoneTest {
    ...
    void legacyMapperEmitsAlmatyOffsetForLocalDateTime() throws Exception {
        ...
        assertTrue(json.contains("+05:00"),
                "LocalDateTime JSON must declare Asia/Almaty offset, got: " + json);
        assertTrue(json.contains("2026-06-15T14:30:00"),
