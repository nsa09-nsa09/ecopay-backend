...
/**
 * Base for integration tests: boots the full Spring context against a real PostgreSQL
 * (Testcontainers), so Flyway migrations (incl. V7 seed + V8 append-only triggers), real
 * transactions and SQL constraints are exercised — unlike the Mockito unit tests. The dev mock
 * payment gateway and the phone dev-bypass code are enabled so the chain can run without external
 * services.
 *
 * <p>The PostgreSQL container uses the "singleton container" pattern (started once in a static
 * initializer, never stopped) so several @SpringBootTest classes can share the same DB without the
 * JVM tearing it down between them. Spring caches contexts across classes — if the container were
 * stopped via JUnit lifecycle, a reused context would point at a dead connection pool.
 */
@SpringBootTest
public abstract class AbstractIntegrationTest {
  ...
  static void properties(DynamicPropertyRegistry registry) {
    ...
    registry.add(
        "app.security.field-encryption-key", () -> "MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=");
