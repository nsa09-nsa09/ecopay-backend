...
class PayoutBatchIntegrationTest extends AbstractIntegrationTest {
  ...
  @AfterEach
  void isolateRemainingPayouts() {
    for (Long id : created) {
      jdbc.update(
          "UPDATE payouts SET status = 'REQUIRES_REVIEW' WHERE id = ? AND status <> 'SUCCESS'", id);
      jdbc.update(
          "UPDATE payout_batches SET status = 'REQUIRES_REVIEW' WHERE id = (SELECT payout_batch_id FROM payouts WHERE id = ?) AND status <> 'SUCCESS'",
          id);
    }
  }

  @Test
  void singletonWaitsUntilCoalescingEndsThenPaysOnce() {
  ...
  @Test
  void compatiblePayoutsProduceOneProviderTransferAndExactLedger() {
    User owner = owner();
  ...
  @Test
  void differentOwnersAndCurrenciesAreNeverCombined() {
  ...
  @Test
  void pendingReconcilesToSuccessWithoutResendEvenAfterDuplicateCallback() {
    User owner = owner();
  ...
  @Test
  void refundBeforeBatchCreationReducesFrozenAmount() {
    User owner = owner();
    Payout a = due(owner, "KZT");
    due(owner, "KZT");
    refund(a, "975.00");
  ...
  @Test
  void refundAfterCommittedClaimBeforeProviderResponseRequiresClawback() {
    User owner = owner();
    Payout a = due(owner, "KZT");
    due(owner, "KZT");
    doAnswer(
  ...
  @Test
  void refundWhileProviderPendingPreservesSettlementAndReviewFlag() {
    User owner = owner();
    Payout a = due(owner, "KZT");
    due(owner, "KZT");
    pending();
  ...
  @Test
  void refundAfterSuccessDoesNotRewriteLedger() {
    User owner = owner();
    Payout a = due(owner, "KZT");
    due(owner, "KZT");
    service.processPendingPayouts();
  ...
  @Test
  void transportRetryUsesSameBatchAndFrozenPayloadAfterRefundAndCardChange() {
    User owner = owner();
    Payout a = due(owner, "KZT");
    due(owner, "KZT");
    ...
    refund(a, "975.00");
    jdbc.update(
        "UPDATE payout_methods SET provider_card_token = 'changed-token' WHERE user_id = ?",
        owner.getId());
    clock.advance(Duration.ofMinutes(6));
  ...
  @Test
  void ambiguousProviderWithoutReplayGuaranteeGoesToReviewWithoutResend() {
    User owner = owner();
    Payout a = due(owner, "KZT");
    due(owner, "KZT");
  ...
  @Test
  void concurrentSchedulersCannotCreateDuplicateBatches() throws Exception {
    User owner = owner();
    Payout a = due(owner, "KZT");
    due(owner, "KZT");
  ...
  @Test
  void databaseRejectsMutationOfSubmittedPayloadAndInvalidRoomCounts() {
    User owner = owner();
    Payout a = due(owner, "KZT");
    due(owner, "KZT");
    service.processPendingPayouts();
    ...
    for (int invalid : List.of(0, 3, 4, 5)) {
      assertThrows(
          ...
          () ->
              jdbc.update(
                  "UPDATE rooms SET existing_members_count = ? WHERE id = ?",
                  invalid,
                  a.getRoom().getId()));
    }
    assertThrows(
        ...
        () ->
            jdbc.update(
                "UPDATE rooms SET max_members = 2, existing_members_count = 2 WHERE id = ?",
                a.getRoom().getId()));
  }
                ...
                status -> {
                  Room room =
                      rooms.save(
                          Room.builder()
                              .owner(owner)
                              .service(services.findById(2L).orElseThrow())
                              ...
                              .startDate(LocalDateTime.now(clock).minusDays(31))
                              .build());
                  User guest =
                      users.save(
                          User.builder()
                  ...
                  RoomMember member =
                      members.save(
                          RoomMember.builder()
                              ...
                              .ownerAccessConfirmedAt(LocalDateTime.now(clock))
                              .memberConfirmedAt(LocalDateTime.now(clock))
                              .build());
                  PaymentIntent intent =
                      intents.save(
                          PaymentIntent.builder()
                              .roomMember(member)
                              .user(guest)
                              .amount(new BigDecimal("1950.00"))
                              .commissionAmount(new BigDecimal("450.00"))
                              .status(PaymentIntentStatus.SUCCESS)
                              .capturedAt(LocalDateTime.now(clock).minusDays(30))
  ...
  private void refund(Payout payout, String amount) {
    ...
    new TransactionTemplate(transactions)
        .executeWithoutResult(
            status -> {
              PaymentIntent intent =
                  intents.findById(payout.getTriggeringPaymentIntent().getId()).orElseThrow();
  ...
  private Payout reload(Payout p) {
    return payouts.findById(p.getId()).orElseThrow();
  }

  private PayoutBatch batch(Payout p) {
    return batches.findById(reload(p).getPayoutBatch().getId()).orElseThrow();
  }

  private void assertLedgerMatches(GatewayPayoutRequest request, int children) {
    BigDecimal total =
        jdbc.queryForObject(
            "SELECT SUM(l.amount) FROM money_ledger_entries l JOIN payouts p ON p.id = l.payout_id WHERE p.payout_batch_id = ? AND l.entry_type = 'PAYOUT'",
            BigDecimal.class,
            request.getPayoutId());
    ...
    assertEquals(
        children,
        jdbc.queryForObject(
            "SELECT COUNT(*) FROM money_ledger_entries l JOIN payouts p ON p.id = l.payout_id WHERE p.payout_batch_id = ? AND l.entry_type = 'PAYOUT'",
            Integer.class,
            request.getPayoutId()));
  }
  ...
  static class TestClock extends Clock {
    final AtomicReference<Instant> now =
        new AtomicReference<>(Instant.parse("2020-01-01T00:00:00Z"));
